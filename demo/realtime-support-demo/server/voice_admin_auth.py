"""Password protection for the production voice admin dashboard."""
...
def admin_secret() -> str:
    return os.environ.get("REALTIME_SALES_ADMIN_SECRET", "").strip()
...
def require_admin_auth(request: Request) -> None:
    """Reject unless REALTIME_SALES_ADMIN_SECRET matches the request token."""
    secret = admin_secret()
    if not secret:
        raise HTTPException(
            status_code=503,
