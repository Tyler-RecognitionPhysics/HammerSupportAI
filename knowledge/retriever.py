"""Grounded retrieval over allowlisted wiki + raw/hammer-data + optional learned playbook.

NotebookLM-style behavior for demos:
- Full demo corpus indexed (wiki allowlist + all hammer markdown + ``knowledge/playbook/approved.md``
  when present, and optionally ``staging.md`` — disable with ``HAMMER_LEARNING_INDEX_STAGING=0``)
- SQLite FTS5 BM25 when `knowledge/data/company_kb.sqlite` exists
- In-memory BM25 + substring fallback when SQLite is missing or sparse
- Query expansion (product names, dealership synonyms) and multi-query fusion
- Never returns empty for a non-empty user question (entity-page fallback)
"""
...
# Must match wiki/company-voice-demo-canonical.md allowlist.
ALLOWED_WIKI_FILES: Tuple[str, ...] = (
    "company-voice-demo-canonical.md",
    "demo-public-site-copy.md",
    "entity-hammer-ai.md",
    "source-hammer-data.md",
)

ENTITY_DOC_ID = "entity-hammer-ai.md"
...
# Domain expansions so conversational queries still hit PDF/transcript wording.
_TERM_EXPANSIONS: dict[str, tuple[str, ...]] = {
    "facebook": ("meta", "aia", "marketplace", "instagram", "ads", "sponsored"),
    "meta": ("facebook", "aia", "instagram", "ads"),
    "aia": ("facebook", "meta", "inventory", "sponsored", "ads", "automotive", "marketplace", "carousel"),
    ...
    "crm": ("vin", "solutions", "dealertrack", "tekion", "cdk", "eleads", "dealercenter"),
    "integration": ("crm", "integrate", "dealertrack", "tekion", "vin", "cdk"),
    "integrations": ("crm", "dealertrack", "tekion", "vin", "cdk"),
    "lead": ("leads", "internet", "follow", "followup", "follow-up", "bdc"),
    "leads": ("lead", "internet", "follow", "followup", "follow-up", "bdc"),
    "shopping": ("competitors", "compare", "rooftop", "rooftops", "listings", "inventory"),
    "shop": ("shopping", "competitors", "compare"),
    "compare": ("shopping", "competitors", "rooftops", "inventory"),
    "competitors": ("shopping", "rooftops", "dealers", "listings", "internet"),
    "rooftops": ("competitors", "shopping", "dealer", "dealership"),
    "yield": ("conversion", "leads", "paid", "roi", "internet"),
    "followup": ("follow-up", "follow", "nurture", "revival", "cold", "dead"),
    "follow-up": ("followup", "follow", "nurture", "revival"),
    "appointment": ("appointments", "book", "booking", "schedule", "show"),
    "pricing": ("price", "cost", "month", "rooftop", "contract", "trial"),
    "price": ("pricing", "cost", "month", "rooftop"),
    "inventory": ("vehicles", "stock", "feed", "listing", "vin"),
    "vehicle": ("inventory", "cars", "suv", "truck"),
    "call": ("voice", "transcript", "listen", "phone", "voicemail"),
    "voice": ("call", "transcript", "listen", "agentic"),
    "agent": ("agentic", "drive", "chatbot", "bot", "assistant"),
    "agentic": ("agent", "call", "transcript", "execute"),
    "drive": ("hammer", "agent", "sales", "rep"),
    "hammer": ("hammertime", "dealership", "dealer"),
    "dealership": ("dealer", "rooftop", "store", "automotive"),
    "dealer": ("dealership", "rooftop", "store"),
    "after": ("hours", "night", "evening", "24", "247"),
    "hours": ("after", "night", "business", "247"),
    "widget": ("chat", "website", "web"),
    "dashboard": ("reporting", "metrics", "analytics", "fire"),
    "trade": ("trade-in", "appraisal", "photo"),
    "credit": ("app", "application", "finance"),
    "spanish": ("french", "multilingual", "language"),
    "french": ("spanish", "multilingual", "language"),
    "podcast": ("washburn", "mcintyre", "dealerbids", "bdc", "150"),
    "onboarding": ("setup", "72", "account", "manager"),
    "setup": ("onboarding", "72", "hours"),
    "trial": ("approve", "renewal", "month"),
    "renewal": ("trial", "approve", "card"),
}
...
class KnowledgeRetriever:
    def __init__(
        self,
        wiki_dir: Path,
        hammer_raw_dir: Path | None = None,
        db_path: Path | None = None,
        playbook_md_path: Path | None = None,
    ) -> None:
        self.wiki_dir = Path(wiki_dir).resolve()
        self.hammer_raw_dir = Path(hammer_raw_dir).resolve() if hammer_raw_dir else None
        repo = self.wiki_dir.parent
        self._playbook_md_path = Path(playbook_md_path).resolve() if playbook_md_path else None
        self.db_path = Path(db_path).resolve() if db_path else (repo / "knowledge" / "data" / "company_kb.sqlite")
    ...
    def _build_memory_index(self) -> None:
        ...
        if self.hammer_raw_dir and self.hammer_raw_dir.is_dir():
            root = self.hammer_raw_dir.resolve()
            for path in sorted(root.rglob("*.md")):
                rel_parts = path.relative_to(root).parts
                if any(p.startswith(".") for p in rel_parts):
                    continue
                doc_id = "raw/hammer-data/" + path.relative_to(root).as_posix()
                try:
        ...
        pb = self._playbook_md_path or (self.wiki_dir.parent / "knowledge" / "playbook" / "approved.md")
        pb = Path(pb).resolve()
        if pb.is_file():
            try:
        ...
        index_staging = os.environ.get("HAMMER_LEARNING_INDEX_STAGING", "1").strip().lower() not in (
            "0",
            "false",
            "no",
        )
        if index_staging:
            st_path = self.wiki_dir.parent / "knowledge" / "playbook" / "staging.md"
            if st_path.is_file():
                try:
                    self._ingest_text("playbook/staging.md", st_path.read_text(encoding="utf-8"))
                except OSError:
                    pass

        self._N = len(self._corpus)
        if self._N == 0:
            raise ValueError("No indexable chunks in wiki + hammer-data corpus")
    ...
    def search(self, query: str, k: int = 8) -> List[Tuple[Chunk, float]]:
        """Retrieve top-k grounded passages for a user or tool query."""
        q = query.strip()
        if not q:
            return []
...
class WikiRetriever(KnowledgeRetriever):
    """Alias kept for existing demo imports."""


# Back-compat for type hints in demos that import Chunk only from retrieval module.
__all__ = ["ALLOWED_WIKI_FILES", "Chunk", "KnowledgeRetriever", "WikiRetriever", "ENTITY_DOC_ID"]
